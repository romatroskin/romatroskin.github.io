...
must_haves:
  truths:
    ...
    - "Logo image has explicit dimensions to prevent CLS"
  artifacts:
    - path: "src/App.tsx"
      provides: "Optimized hero section with explicit dimensions"
...
<objective>
Optimize Core Web Vitals (LCP, CLS) through explicit dimensions, preloading, and layout stability.

Purpose: Achieve Lighthouse 90+ by fixing the primary CWV metrics that affect score.
Output: Stable layout with no CLS, fast LCP through preloading, optimized hero section.
</objective>
...
<task type="auto">
  <name>Task 1: Add explicit dimensions and preload hints</name>
  <files>
    - index.html
    - src/App.tsx
  </files>
  <action>
    ...
    2. Update src/App.tsx logo image to include explicit dimensions:
       - Add width and height attributes to prevent CLS:
         ...
         />
         ```
       - The exact dimensions should match the CSS-defined size or aspect ratio
  ...
  <verify>
    ...
    - App.tsx img tag has width and height attributes
...
- App.tsx logo has explicit width/height attributes
