...
<task type="auto">
  <name>Task 1: Fix scroll binding timing in App.tsx</name>
  <files>src/App.tsx</files>
  <action>
The useScroll hook uses `container: parallaxRef?.current?.container` which is `undefined` on initial render. This causes scrollY to never update because the scroll listener isn't attached to the parallax container.

**Root cause:** `parallaxRef.current` is null during the initial render when useScroll is called. The hook captures this null value and never rebinds when the ref becomes available.

**Fix approach:** Use a state-based approach to trigger re-initialization of useScroll after Parallax mounts.

1. Add a containerRef state that updates after Parallax mounts:
   ```typescript
   const [scrollContainer, setScrollContainer] = useState<HTMLDivElement | null>(null);

   // After Parallax mount, capture its container
   useEffect(() => {
     if (parallaxRef.current?.container) {
       setScrollContainer(parallaxRef.current.container as HTMLDivElement);
     }
   }, []);
   ```

2. Pass scrollContainer to useScroll:
   ```typescript
...
**Alternative approach (simpler):** Use useScroll without container option to track window scroll, but this requires the waves to be outside the Parallax scroll container. Evaluate which approach is cleaner.

**Verification:** Console.log scrollY.get() during scroll to confirm values change from 0.
  </action>
  <verify>
1. Run `npm run dev`
2. Open browser console
3. Add temporary log in Wave calculateWavePoints: `console.log('height:', getAnimatedValue(heightProp))`
4. Scroll the page - height value should change as you scroll
  </verify>
  <done>scrollY updates correctly as user scrolls the parallax container</done>
</task>
